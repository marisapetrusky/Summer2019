...
void fcn(int& npar, double* gin, double& result, double *par, int iflag)
{
	...
	for (int ievt = 0; ievt < entries; ievt++)
	{
		...
		chi2x = 1e7*pow(x1_fit - x1_coord,2); // Scale Chi2 because residual is too small
		chi2y = 1e7*pow(y1_fit - y1_coord,2);
		chi2 += (chi2x + chi2y)/2;
	}	
...
void OffsetCalc_MIGRAD()
{
	Tin->GetEntry(0);
	Tin->SetBranchAddress("x1_coord",&x1_coord);
	Tin->SetBranchAddress("y1_coord",&y1_coord);
	...
	Double_t minTrans = -1e-3;
	Double_t maxTrans = 1e-3;
	Double_t stepTrans = 1e-6;

	gMinuit->DefineParameter(0,"x2off",1.0001e-3,stepTrans,minTrans,maxTrans);
	gMinuit->DefineParameter(1,"x3off",1.0001e-3,stepTrans,minTrans,maxTrans);
	gMinuit->DefineParameter(2,"y2off",1.0001e-3,stepTrans,minTrans,maxTrans);
	gMinuit->DefineParameter(3,"y3off",1.0001e-3,stepTrans,minTrans,maxTrans);

	gMinuit->mnexcm("CALL FCN",arglist,1,iflag);
	arglist[0] = 10000; // Max number of calls
